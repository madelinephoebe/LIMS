...
<div class="col-sm-4 col-md-6 col-lg-3 mt-4 mb-5">
 ...
 <button type="submit" class="add-btn btn btn-success" id="submitButton">Submit</button>
  </form>
</div><script>
...
function resetItemNamesDropdown() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        fetchProfessorSchedule(); // Fetch professor schedule when the page loads

        const form = document.getElementById('studentForm');
        form.addEventListener('submit', function (event) {
            event.preventDefault(); // Prevent default form submission
            validateBorrowingTime(); // Validate borrowing time before submitting the form
        });

        showFormSection(); // Call the showFormSection function immediately after the DOM is loaded
    });

    function fetchProfessorSchedule() {
  fetch('fetchProfessorSchedule.php')
    .then(response => {
      if (!response.ok) {
    ...
    .then(schedule => {
      ...
      // Populate dropdown with rooms fetched from the professor schedule
      schedule.forEach(professor => {
        const option = document.createElement('option');
        option.value = professor.room;
        option.text = professor.room;
        roomDropdown.appendChild(option);
      });
    })
    .catch(error => console.error('Error fetching professor schedule:', error));
}

    function validateBorrowingTime() {
        const selectedRoom = document.getElementById('room').value; // Get selected room from the form
        const selectedTime = document.getElementById('subjectTime').value; // Get selected time from the form

        // Check if there is a professor assigned to the selected room during the specified time
        const professorAssigned = window.professorSchedule.some(professor => {
            return professor.room === selectedRoom && professor.time === selectedTime;
        });

        if (!professorAssigned) {
            // If no professor is assigned to the selected room and time, display an error message
            alert('Borrowing is not allowed for the selected room and time');
        } else {
            // If a professor is assigned to the selected room and time, proceed with form submission
            submitForm();
        }
    }
</script>
